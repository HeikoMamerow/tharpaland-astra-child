 ...
 *
 * @package advanced-notice-in-menu
 */

/**
 * Function for shortcode "em_advanced_notice".
 *
 * @return string|void
 */
function em_advanced_notice_func() {
	...
	$scope_today = gmdate( 'Y-m-d' );
	$scope_later = gmdate( 'Y-m-d', strtotime( '+21 day' ) );
	...
	$em_events = EM_Events::get(
		array(
			...
			'category'    => '29', // Event category 'Wochenprogramm'.
		)
	);
	...
	if ( ! empty( $em_events ) ) {
		...
		foreach ( $em_events as $em_event ) {
			$events[] = array(
				...
				'day'           => $intl_date_formatter->format( strtotime( $em_event->start_date ) ),
		...
		$already_existing_events_basket = array();

		foreach ( $events as $event ) {

			$event_control_number = $event['day_number'] . '-' . $event['recurrence_id'];

			if ( ! in_array( $event_control_number, $already_existing_events_basket, true ) ) {

				$already_existing_events_basket[] = $event_control_number;
				...
				$string .= '<span class="menu-link-flex-item2">' . gmdate( 'G:i', strtotime( $event['start_time'] ) ) . '</span> ';
