...
if ( ! function_exists( 'astra_single_get_post_meta' ) ) {
	...
	function astra_single_get_post_meta( $echo = true ) {
		...
		if ( is_array( $post_meta ) && ( in_array( $current_post_type, $post_type_array ) || 'attachment' === $current_post_type ) && $enable_meta ) {
			...
			$output_str = astra_get_post_meta( (string) $post_meta );
			if ( ! empty( $output_str ) ) {
...
<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
	<p><?php esc_html_e( 'Pingback:', 'astra' ); ?> <?php comment_author_link(); ?>
		<?php edit_comment_link( __( '(Edit)', 'astra' ), '<span class="edit-link">', '</span>' ); ?></p>
</li>
<?php
				break;
...
<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
	...
	<article id="comment-<?php comment_ID(); ?>" class="ast-comment">
		<div class='ast-comment-avatar-wrap'><?php echo get_avatar( $comment, 50 ); ?></div>
		<!-- Remove 1px Space
						-->
		<div class="ast-comment-data-wrap">
			...
			<section class="ast-comment-content comment">
				...
				<p class="ast-highlight-text comment-awaiting-moderation">
					<?php echo esc_html( astra_default_strings( 'string-comment-awaiting-moderation', false ) ); ?></p>
				<?php endif; ?>
